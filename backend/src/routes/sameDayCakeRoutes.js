  ...
  filename: function (req, file, cb) {
    try {
      ...
      if (!productName) {
        ...
        return cb(null, 'sameday-cake-' + uniqueSuffix + ext);
      }
      ...
      const uniqueSuffix = Date.now() + '-' + Math.round(Math.random() * 1E9);
      const ext = path.extname(file.originalname).toLowerCase();
      cb(null, 'sameday-cake-' + uniqueSuffix + ext);
    }
  }
});
...
router.post('/', (req, res, next) => {
  ...
  upload(req, res, function(err) {
    ...
    // Agora processa a criação do same day cake
    next();
  });
}, async (req, res) => {
  ...
  try {
    ...
    // console.log(`📝 Nome do sameDayCake: ${name}`);

    // Insere no banco
    const [sameDayCakeResult] = await connection.query(
      'INSERT INTO same_day_cakes (name, description, image, is_active) VALUES (?, ?, ?, ?)',
      [String(name).trim(), description?.trim() || '', imageFilename, isActiveVal]
    );

    const sameDayCakeId = sameDayCakeResult.insertId;
      ...
      if (Array.isArray(sizesArray) && sizesArray.length > 0) {
        for (const size of sizesArray) {
          ...
          await connection.query(
            'INSERT INTO same_day_cake_sizes (same_day_cake_id, size, stock, price, is_active) VALUES (?, ?, ?, ?, ?)',
            [sameDayCakeId, size.size, size.stock || 0, size.price || 0, sizeIsActive]
          );
        }
      }
    }
    ...
    await connection.rollback();
    console.error('❌ Erro ao criar sameDayCake:', err);
    res.status(500).json({ 
      success: false, 
...
router.put('/:id', (req, res, next) => {
  ...
  try {
    ...
    const sameDayCakeId = req.params.id;
    const { name, description, sizes, existingImage, is_active } = req.body;

    const isActiveVal = (is_active === 'true' || is_active === true || is_active === '1' || is_active === 1 || is_active === undefined) ? 1 : 0;

    // console.log('📦 Update - Dados:', { sameDayCakeId, name, description, sizes, existingImage });

    // Verifica se o sameDayCake existe
    const [existingCakes] = await connection.query(
      'SELECT * FROM same_day_cakes WHERE id = ?', 
      [sameDayCakeId]
    );
    ...
    // Atualiza cake
    await connection.query(
      'UPDATE same_day_cakes SET name = ?, description = ?, image = ?, is_active = ? WHERE id = ?',
      [String(name).trim(), description?.trim() || '', imageFilename, isActiveVal, sameDayCakeId]
    );

    // Remove sizes antigos
    await connection.query('DELETE FROM same_day_cake_sizes WHERE same_day_cake_id = ?', [sameDayCakeId]);
    ...
    await connection.rollback();
    console.error('❌ Erro ao atualizar sameDayCake:', err);
    res.status(500).json({ 
      success: false, 
...
router.get('/', async (req, res) => {
  try {
    const [same_day_cakes] = await pool.query('SELECT * FROM same_day_cakes ORDER BY id');
    const [sizes] = await pool.query('SELECT * FROM same_day_cake_sizes ORDER BY id');

    const result = same_day_cakes.map(sameDayCake => ({
      ...sameDayCake,
      sizes: sizes.filter(s => s.same_day_cake_id === sameDayCake.id).sort((a, b) => a.id - b.id)
    }));

    res.json({ success: true, same_day_cakes: result });
  } catch (err) {
    console.error(err);
    res.status(500).json({ 
      success: false, 
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const [same_day_cakes] = await pool.query('SELECT * FROM same_day_cakes WHERE id = ?', [sameDayCakeId]);
    
    if (same_day_cakes.length === 0) {
      return res.status(404).json({ 
        success: false, 
        error: 'ケーキが見つかりません' 
      });
    ...
    const [sizes] = await pool.query(
      'SELECT * FROM same_day_cake_sizes WHERE same_day_cake_id = ? ORDER BY id', 
      [sameDayCakeId]
    );
    ...
    res.json({ success: true, sameDayCake: result });
  } catch (err) {
    console.error(err);
    res.status(500).json({ 
      success: false, 
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    await connection.query('DELETE FROM same_day_cake_sizes WHERE same_day_cake_id = ?', [sameDayCakeId]);
    await connection.query('DELETE FROM same_day_cakes WHERE id = ?', [sameDayCakeId]);
    ...
    await connection.rollback();
    console.error('❌ Erro ao deletar sameDayCake:', err);
    res.status(500).json({ 
      success: false, 
